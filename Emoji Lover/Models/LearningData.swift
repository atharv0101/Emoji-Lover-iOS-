...
// In LearningData.swift
struct LearningCategory: Identifiable, Equatable {
    let id = UUID()
    let title: String
    let icon: String
...
// In LearningData.swift, at the very bottom

// 1. A new category that mixes items from all other categories.
let mixedCategory = LearningCategory(
    title: "Mixed",
    icon: "🎲",
    // Combine a few items from each category and shuffle them.
    items: Array(
        (fruits.prefix(4) +
         animals.prefix(4) +
         numbers.prefix(4) +
         shapes.prefix(2) + // Shapes has fewer items
         colors.prefix(4) +
         vegetables.prefix(4))
        .shuffled()
    )
...
// 2. A new master list for our game that includes the "Mixed" option.
let gameCategories: [LearningCategory] = [mixedCategory] + allCategories
